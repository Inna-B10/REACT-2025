...
export function List() {
  ...
  const filterdList = articleResponse.articles
    .filter((article) =>
      tagFilter === "" ? true : article.tagList.includes(tagFilter),
    )
    .filter((article) =>
      customArticleFilter(article, searchTerm.toLowerCase()),
    );
  ...
  return (
    <div>
      <div className="p-1 flex justify-center gap-1">
        <input
          className="bg-neutral-100 border border-neutral-200 shadow-sm rounded-l-full pl-4 py-1"
          placeholder="Search.."
          type="text"
        ...
        <select
          defaultValue=""
          className="bg-neutral-100 border border-neutral-200 shadow-sm rounded-r-full pr-4 py-1"
    ...
    <article className="flex flex-col gap-4 px-4 mx-auto max-w-2xl">
      ...
      <footer className="flex justify-between items-end">
        ...
        <div className="text-xs flex flex-col">
          <p className="flex grow gap-2 justify-between">
            Authored:{" "}
            <span>
          ...
          <p className="flex grow gap-2 justify-between">
            Updated:{" "}
            <span>
              {new Date(props.article.updatedAt).toLocaleDateString()}
            </span>
          </p>
        </div>
      </footer>
